...
public class HttpsRequest implements IServiceRequest{
    ...
    public String sendPostMap(String url,  Map<String,Object> map) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        ...
        //将要提交给API的数据对象转换成XML格式数据Post给API
        String postDataXML = mapToXML(map);
        
        //String postDataXML = mapToXmlISO(map);
     ...
     * map to xml
     * */
    public static String mapToXmlISO(Map<String,Object> map){
    	...
    	String endXml = "";
    	try {
    		endXml = new String(xml.toString().getBytes(), "ISO8859-1");//搞定中文乱码造成的签名错误问题
    		//Log.i("Signature", "endXml="+endXml);
		} catch (Exception e) {
			e.printStackTrace();
		}
    	return endXml;
    ...
    public static String mapToXMLWithCDATA(Map<String,Object> map){
    	StringBuilder xml = new StringBuilder();
    	xml.append("<xml>");
    	for(Map.Entry<String,Object> entry:map.entrySet()){
    		xml.append("<"+entry.getKey()+">");
